...
namespace
{
  ...
  // ATL-free replacements for CA2W / CW2A
  ...
  const std::wstring CREDENTIAL_FILTER = s2ws(ELEMENT_PREFERENCES_KEY_PREFIX + '*');
  ...
  PBYTE FlutterSecureStorageWindowsPlugin::GetEncryptionKey()
  {
      ...
      PBYTE AesKey;
      PCREDENTIALW pcred;
      std::wstring target_name = s2ws("key_" + ELEMENT_PREFERENCES_KEY_PREFIX);
  ...
  std::optional<std::string> FlutterSecureStorageWindowsPlugin::Read(const std::string &key)
  {
      ...
      if (!fs.good()) {
          //Backwards comp.
          PCREDENTIALW pcred;
          std::wstring target_name = s2ws(key);
          bool ok = CredReadW(target_name.c_str(), CRED_TYPE_GENERIC, 0, &pcred);
          if (ok)
          {
...
#pragma warning(pop)
  ...
  flutter::EncodableMap FlutterSecureStorageWindowsPlugin::ReadAll()
  {
    ...
    {
      auto pcred = pcreds[i];
      std::string target_name = ws2s(pcred->TargetName);
      auto val = std::string((char*)pcred->CredentialBlob);
